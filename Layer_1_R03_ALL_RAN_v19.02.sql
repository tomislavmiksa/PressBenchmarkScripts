...
Select distinct	FileId,SessionId,TestId,NetworkId,MsgTime
			   ...
			   ,cast(null as varchar(255))		as vdf_cn
...
IF EXISTS (SELECT * FROM   sysobjects WHERE name = N'VDF_DE_4G_Cells')      
	Begin
		UPDATE NEW_RAN_RAW_2019_NEW
		SET vdf_cn = b.CELL
		FROM NEW_RAN_RAW_2019_NEW a
		LEFT OUTER JOIN VDF_DE_4G_Cells 
		b ON a.Cid = b.ECI
		WHERE MNC = 2 --and technology = 'LTE%'
	End
...
Declare @cVDF as varchar(30)   = ''
Declare @pVDF as varchar(30)   = ''
Declare @rVDF as varchar(2000) = ''
Declare @tVDF as varchar(2000) = ''
...
PRINT(CONVERT(VARCHAR,GETDATE(),120) +  '  - CREATING TABLE NEW_RAN_SESSION_2019 AND TABLE NEW_RAN_TEST_2019...')
...
DECLARE TN_cursor CURSOR FAST_FORWARD FOR 
    SELECT SessionId,TestId,MCC,MNC,CGI,LAC,PCI,CID,BCCH,BSIC,sc1,sc2,sc3,sc4,technology,vdf_cn
    from
...
while 1=1
Begin
	Fetch next FROM TN_cursor into @cSessionId, @cTestId, @cMCC, @cMNC, @cCGI, @cLAC, @cPCI, @cCID, @cBCCH, @cBSIC, @csc1, @csc2, @csc3, @csc4, @ctec, @cVDF
	-- WRITE SESSION TABLE
	...
	if @cTestId <> @pTestId
		begin
			...
			Set @tVDF = CASE WHEN @pVDF  <> '' and @pVDF  is not null then '[' + @pVDF  + '],' ELSE '' END
			Set @pTestId = @cTestId
		end
	-- CALCULATE MCC
    ...
    if @cCID<>@pCID and @cCID <> '' and @cCID is not null
		begin
			...
			-- CALCULATE TECHNOLOGY, LAC, CID, BCCH
			set @ptec = @ctec
			Set @rtec = @rtec + '['+@ctec + ',' + @cLAC + ',' + @cCID +  + ',' + @cBCCH  + '],'
			Set @ttec = @ttec + '['+@ctec + ',' + @cLAC + ',' + @cCID +  + ',' + @cBCCH  + '],'

			-- CALCULATE VDF CELL NAME
			set @pVDF = CASE WHEN @cVDF is null THEN '' ELSE @cVDF END 
			Set @rVDF = @rVDF + '['+@cVDF +'], '
			Set @tVDF = @tVDF + '['+@cVDF +'], '
		end
	-- CALCULATE BCCH
...
PRINT(CONVERT(VARCHAR,GETDATE(),120) +  '  - CALCULATING UL LTE VARIABLES...')
	...
	IF OBJECT_ID ('tempdb..#warbt' ) IS NOT NULL DROP TABLE #warbt
	select SessionId,
		   TestId,
		   AVG(NumRBs) as AvgRB
	into #warbt
	...
	UPDATE NEW_RAN_TEST_2019
	SET LTE_DL_AvgRB						= b.AvgRB
	FROM NEW_RAN_TEST_2019 a
	LEFT OUTER JOIN #warbt b on a.SessionId = b.SessionId and a.TestId = b.TestId

	IF OBJECT_ID ('tempdb..#warbs' ) IS NOT NULL DROP TABLE #warbs
	select SessionId,
		   AVG(NumRBs) as AvgRB
	into #warbs
	from LTEPDSCHStatisticsInfo
	...
	SET LTE_DL_AvgRB						= b.AvgRB
	FROM NEW_RAN_SESSION_2019 a
	LEFT OUTER JOIN #warbs b on a.SessionId = b.SessionId
...
GO
    ...
    UPDATE NEW_RAN_SESSION_2019
	SET LTE_DL_AvgRB = null
	WHERE (LTE_DL_MaxDLNumCarriers = 1 and LTE_DL_AvgRB > 100) or (LTE_DL_MaxDLNumCarriers = 2 and LTE_DL_AvgRB > 200) or (LTE_DL_MaxDLNumCarriers = 3 and LTE_DL_AvgRB > 300) or (LTE_DL_MaxDLNumCarriers = 4 and LTE_DL_AvgRB > 400)
...
-- select * from NEW_RAN_TEST_2019 where MNC like '%2%'
-- select * from #TA_LTE where AvgTA is not null
-- select top 500 * from NEW_RAN_RAW_2019_NEW where MNC like '2' and vdf_cn is not null
-- select * from NEW_RAN_Test_2019 where mnc like '%2%' and VDF_CELL like '%B%'
-- select * from NEW_RAN_SESSION_2019 where MNC like '%2%'
